...
impl Adc {
    ...
    #[inline(always)]
    pub const fn awdltr(self, n: usize) -> crate::common::Reg<regs::Awdltr, crate::common::RW> {
        assert!(n < 3usize);
    ...
    #[inline(always)]
    pub const fn awdhtr(self, n: usize) -> crate::common::Reg<regs::Awdhtr, crate::common::RW> {
        assert!(n < 3usize);
...
pub mod regs {
    ...
    impl Awd2cr {
        ...
        #[must_use]
        #[inline(always)]
        pub const fn awdch(&self) -> u16 {
            let val = (self.0 >> 0usize) & 0x3fff;
            val as u16
        }
        #[doc = "Analog watchdog 2 channel selection"]
        #[inline(always)]
        pub const fn set_awdch(&mut self, val: u16) {
    ...
    impl defmt::Format for Awd2cr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Awd2cr {{ awdch: {=u16:?} }}", self.awdch())
        }
    }
    #[doc = "analog watchdog 3 configuration register"]
    #[repr(transparent)]
    ...
    impl defmt::Format for Awd3cr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Awd3cr {{ awdch: {=u16:?} }}", self.awdch())
        }
    }
    #[doc = "analog watchdog 1-3 higher threshold register"]
    #[repr(transparent)]
    #[derive(Copy, Clone, Eq, PartialEq)]
    pub struct Awdhtr(pub u32);
    impl Awdhtr {
        #[doc = "Analog watchdog 1-3 higher threshold."]
        #[must_use]
        #[inline(always)]
    ...
    impl Default for Awdhtr {
        #[inline(always)]
        fn default() -> Awdhtr {
            Awdhtr(0)
        }
    }
    impl core::fmt::Debug for Awdhtr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Awdhtr")
                .field("htr", &self.htr())
    ...
    #[cfg(feature = "defmt")]
    impl defmt::Format for Awdhtr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Awdhtr {{ htr: {=u32:?}, awdfilt: {=u8:?} }}",
                self.htr(),
                self.awdfilt()
            )
        }
    ...
    impl Default for Awdltr {
        #[inline(always)]
        fn default() -> Awdltr {
            Awdltr(0)
        }
    }
    impl core::fmt::Debug for Awdltr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Awdltr").field("ltr", &self.ltr()).finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Awdltr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Awdltr {{ ltr: {=u32:?} }}", self.ltr())
        }
    }
    #[doc = "calibration factors"]
    #[repr(transparent)]
    ...
    impl defmt::Format for Cfgr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cfgr {{ dmngt: {:?}, res: {:?}, extsel: {=u8:?}, exten: {:?}, ovrmod: {:?}, cont: {=bool:?}, autdly: {=bool:?}, discen: {=bool:?}, discnum: {=u8:?}, jdiscen: {=bool:?}, awd1sgl: {:?}, awd1en: {=bool:?}, jawd1en: {=bool:?}, jauto: {=bool:?}, awd1ch: {=u8:?} }}",
                self.dmngt(),
                self.res(),
                self.extsel(),
                self.exten(),
...
= 000 configuration is compatible with bulb mode."]
    ...
    impl defmt::Format for Cr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cr {{ aden: {=bool:?}, addis: {=bool:?}, adstart: {=bool:?}, jadstart: {=bool:?}, adstp: {:?}, jadstp: {=bool:?}, advregen: {=bool:?}, deeppwd: {=bool:?}, adcal: {=bool:?} }}",
                self.aden(),
                self.addis(),
                self.adstart(),
                self.jadstart(),
    ...
    impl Gcomp {
        #[doc = "Gain compensation coefficient The coefficient is divided by 4096 to get the gain factor ranging from 0 to 39999756"]
        #[must_use]
        #[inline(always)]
...
bits."]
    ...
    impl Pcsel {
        #[doc = "Channel x (VIN\\[i\\]) preselected"]
        #[must_use]
        #[inline(always)]
    ...
    impl defmt::Format for Pcsel {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Pcsel {{ pcsel[0]: {:?}, pcsel[1]: {:?}, pcsel[2]: {:?}, pcsel[3]: {:?}, pcsel[4]: {:?}, pcsel[5]: {:?}, pcsel[6]: {:?}, pcsel[7]: {:?}, pcsel[8]: {:?}, pcsel[9]: {:?}, pcsel[10]: {:?}, pcsel[11]: {:?}, pcsel[12]: {:?}, pcsel[13]: {:?} }}",
                self.pcsel(0usize),
                self.pcsel(1usize),
                self.pcsel(2usize),
                self.pcsel(3usize),
