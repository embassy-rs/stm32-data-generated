...
pub mod regs {
    ...
    impl Cr1 {
        ...
        #[doc = "Dual mode selection"]
        #[inline(always)]
        pub const fn set_dualmod(&mut self, val: super::vals::Dualmod) {
    ...
    impl defmt::Format for Cr1 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cr1 {{ awdch: {=u8:?}, eocie: {=bool:?}, awdie: {=bool:?}, jeocie: {=bool:?}, scan: {=bool:?}, awdsgl: {=bool:?}, jauto: {=bool:?}, discen: {=bool:?}, jdiscen: {=bool:?}, discnum: {=u8:?}, dualmod: {:?}, jawden: {=bool:?}, awden: {=bool:?} }}",
                self.awdch(),
                self.eocie(),
                self.awdie(),
                self.jeocie(),
    ...
    impl Dr {
        ...
        #[doc = "ADC2 data"]
        #[inline(always)]
        pub const fn set_adc2data(&mut self, val: u16) {
    ...
    impl defmt::Format for Smpr1 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Smpr1 {{ smp[0]: {:?}, smp[1]: {:?}, smp[2]: {:?}, smp[3]: {:?}, smp[4]: {:?}, smp[5]: {:?}, smp[6]: {:?}, smp[7]: {:?} }}",
                self.smp(0usize),
                self.smp(1usize),
                self.smp(2usize),
                self.smp(3usize),
    ...
    impl defmt::Format for Smpr2 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Smpr2 {{ smp[0]: {:?}, smp[1]: {:?}, smp[2]: {:?}, smp[3]: {:?}, smp[4]: {:?}, smp[5]: {:?}, smp[6]: {:?}, smp[7]: {:?}, smp[8]: {:?}, smp[9]: {:?} }}",
                self.smp(0usize),
                self.smp(1usize),
                self.smp(2usize),
                self.smp(3usize),
