...
pub mod regs {
    ...
    impl Cfgr1 {
        ...
        #[doc = "ADC analog watchdog 1 monitored channel selection"]
        #[inline(always)]
        pub const fn set_awdch1ch(&mut self, val: u8) {
    ...
    impl defmt::Format for Cfgr1 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cfgr1 {{ dmaen: {=bool:?}, dmacfg: {:?}, scandir: {=bool:?}, res: {:?}, align: {=bool:?}, extsel: {=u8:?}, exten: {=u8:?}, ovrmod: {=bool:?}, cont: {=bool:?}, wait: {=bool:?}, autoff: {=bool:?}, discen: {=bool:?}, chselrmod: {=bool:?}, awd1sgl: {=bool:?}, awd1en: {=bool:?}, awdch1ch: {=u8:?} }}",
                self.dmaen(),
                self.dmacfg(),
                self.scandir(),
                self.res(),
    ...
    impl defmt::Format for Cfgr2 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cfgr2 {{ ovse: {=bool:?}, ovsr: {=u8:?}, ovss: {=u8:?}, tovs: {=bool:?}, lftrig: {=bool:?}, ckmode: {=u8:?} }}",
                self.ovse(),
                self.ovsr(),
                self.ovss(),
                self.tovs(),
    ...
    impl Chselr {
        ...
        #[doc = "Channel-x selection"]
        #[inline(always)]
        pub const fn set_chsel(&mut self, val: u32) {
            self.0 = (self.0 & !(0x0007_ffff << 0usize)) | (((val as u32) & 0x0007_ffff) << 0usize);
        }
    }
    ...
    impl defmt::Format for Chselr1 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Chselr1 {{ sq1: {=u8:?}, sq2: {=u8:?}, sq3: {=u8:?}, sq4: {=u8:?}, sq5: {=u8:?}, sq6: {=u8:?}, sq7: {=u8:?}, sq8: {=u8:?} }}",
                self.sq1(),
                self.sq2(),
                self.sq3(),
                self.sq4(),
    ...
    impl Dr {
        ...
        #[doc = "ADC group regular conversion data"]
        #[inline(always)]
        pub const fn set_regular_data(&mut self, val: u16) {
    ...
    impl Ier {
        ...
        #[doc = "Channel Configuration Ready Interrupt enable"]
        #[inline(always)]
        pub const fn set_ccrdyie(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 13usize)) | (((val as u32) & 0x01) << 13usize);
        }
    }
    ...
    impl Isr {
        ...
        #[doc = "Channel Configuration Ready flag"]
        #[inline(always)]
        pub const fn set_ccrdy(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 13usize)) | (((val as u32) & 0x01) << 13usize);
        }
    }
    ...
    impl defmt::Format for Smpr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Smpr {{ smp1: {:?}, smp2: {:?}, smpsel[0]: {=bool:?}, smpsel[1]: {=bool:?}, smpsel[2]: {=bool:?}, smpsel[3]: {=bool:?}, smpsel[4]: {=bool:?}, smpsel[5]: {=bool:?}, smpsel[6]: {=bool:?}, smpsel[7]: {=bool:?}, smpsel[8]: {=bool:?}, smpsel[9]: {=bool:?}, smpsel[10]: {=bool:?}, smpsel[11]: {=bool:?}, smpsel[12]: {=bool:?}, smpsel[13]: {=bool:?}, smpsel[14]: {=bool:?}, smpsel[15]: {=bool:?}, smpsel[16]: {=bool:?}, smpsel[17]: {=bool:?}, smpsel[18]: {=bool:?} }}",
                self.smp1(),
                self.smp2(),
                self.smpsel(0usize),
                self.smpsel(1usize),
