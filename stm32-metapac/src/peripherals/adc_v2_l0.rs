...
pub mod regs {
    ...
    impl Ccr {
        ...
        #[must_use]
        #[inline(always)]
        pub const fn vlcden(&self) -> bool {
            let val = (self.0 >> 24usize) & 0x01;
            val != 0
        }
        #[doc = "VLCD enable"]
        #[inline(always)]
        pub const fn set_vlcden(&mut self, val: bool) {
    ...
    impl defmt::Format for Ccr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Ccr {{ presc: {:?}, vrefen: {=bool:?}, tsen: {=bool:?}, vlcden: {=bool:?}, lfmen: {=bool:?} }}",
                self.presc(),
                self.vrefen(),
                self.tsen(),
                self.vlcden(),
    ...
    impl defmt::Format for Cfgr2 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cfgr2 {{ ovse: {=bool:?}, ovsr: {:?}, ovss: {=u8:?}, tovs: {=bool:?}, ckmode: {:?} }}",
                self.ovse(),
                self.ovsr(),
                self.ovss(),
                self.tovs(),
    ...
    impl defmt::Format for Ier {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Ier {{ adrdyie: {=bool:?}, eosmpie: {=bool:?}, eocie: {=bool:?}, eosie: {=bool:?}, ovrie: {=bool:?}, awdie[0]: {=bool:?}, eocalie: {=bool:?} }}",
                self.adrdyie(),
                self.eosmpie(),
                self.eocie(),
                self.eosie(),
    ...
    impl defmt::Format for Isr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Isr {{ adrdy: {=bool:?}, eosmp: {=bool:?}, eoc: {=bool:?}, eos: {=bool:?}, ovr: {=bool:?}, awd[0]: {=bool:?}, eocal: {=bool:?} }}",
                self.adrdy(),
                self.eosmp(),
                self.eoc(),
                self.eos(),
