...
pub mod regs {
    ...
    impl defmt::Format for Ccr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Ccr {{ dual: {:?}, delay: {=u8:?}, damdf: {:?}, presc: {:?}, vrefen: {=bool:?}, vsenseen: {=bool:?}, vbaten: {=bool:?} }}",
                self.dual(),
                self.delay(),
                self.damdf(),
                self.presc(),
...
= 11 mode, bits 15:8 contains SLV_ADC_DR\\[7:0\\], bits 7:0 contains MST_ADC_DR\\[7:0\\]."]
    ...
    impl Csr {
        ...
        #[doc = "Analog watchdog 1 flag of the slave ADC This bit is a copy of the AWD1 bit in the corresponding ADCx+1_ISR register."]
        #[inline(always)]
        pub const fn set_awd1_slv(&mut self, n: usize, val: bool) {
            assert!(n < 3usize);
            let offs = 23usize + n * 1usize;
            self.0 = (self.0 & !(0x01 << offs)) | (((val as u32) & 0x01) << offs);
        }
    ...
    impl defmt::Format for Csr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                ...
                self.jeoc_slv(),
                self.jeos_slv(),
                self.awd1_slv(0usize),
                self.awd1_slv(1usize),
                self.awd1_slv(2usize),
                self.ldordy_slv()
            )
        }
    }
