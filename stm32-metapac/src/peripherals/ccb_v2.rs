...
pub mod regs {
    ...
    impl core::fmt::Debug for Sr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Sr")
                ...
                .field("tamp_evt[5]", &self.tamp_evt(5usize))
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Sr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Sr {{ opstep: {=u8:?}, operr: {=u8:?}, busy: {=bool:?}, tamp_evt[0]: {=bool:?}, tamp_evt[1]: {=bool:?}, tamp_evt[2]: {=bool:?}, tamp_evt[3]: {=bool:?}, tamp_evt[4]: {=bool:?}, tamp_evt[5]: {=bool:?} }}",
                self.opstep(),
                self.operr(),
                self.busy(),
