...
pub mod regs {
    ...
    impl defmt::Format for Bcr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Bcr {{ mbken: {=bool:?}, muxen: {=bool:?}, mtyp: {:?}, mwid: {:?}, faccen: {=bool:?}, bursten: {=bool:?}, waitpol: {:?}, waitcfg: {:?}, wren: {=bool:?}, waiten: {=bool:?}, extmod: {=bool:?}, asyncwait: {=bool:?}, cpsize: {:?}, cburstrw: {=bool:?}, nblset: {=u8:?} }}",
                self.mbken(),
                self.muxen(),
                self.mtyp(),
                self.mwid(),
    ...
    impl defmt::Format for Bcr1 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Bcr1 {{ mbken: {=bool:?}, muxen: {=bool:?}, mtyp: {:?}, mwid: {:?}, faccen: {=bool:?}, bursten: {=bool:?}, waitpol: {:?}, waitcfg: {:?}, wren: {=bool:?}, waiten: {=bool:?}, extmod: {=bool:?}, asyncwait: {=bool:?}, cpsize: {:?}, cburstrw: {=bool:?}, cclken: {=bool:?}, wfdis: {=bool:?}, nblset: {=u8:?}, fmcen: {=bool:?} }}",
                self.mbken(),
                self.muxen(),
                self.mtyp(),
                self.mwid(),
    ...
    impl defmt::Format for Pcscntr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Pcscntr {{ cscount: {=u16:?}, cntb1en: {=bool:?}, cntb2en: {=bool:?}, cntb3en: {=bool:?}, cntb4en: {=bool:?} }}",
                self.cscount(),
                self.cntb1en(),
                self.cntb2en(),
                self.cntb3en(),
