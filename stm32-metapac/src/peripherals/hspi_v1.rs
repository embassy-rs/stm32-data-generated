...
is forced to 1. Writes to this field are ignored when BUSY├é┬á=├é┬á1 or when FMODE = 11 (Memory-mapped mode)."]
    ...
    impl defmt::Format for Ccr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Ccr {{ imode: {=u8:?}, idtr: {=bool:?}, isize: {=u8:?}, admode: {=u8:?}, addtr: {=bool:?}, adsize: {=u8:?}, abmode: {=u8:?}, abdtr: {=bool:?}, absize: {=u8:?}, dmode: {=u8:?}, ddtr: {=bool:?}, dqse: {=bool:?}, sioo: {=bool:?} }}",
                self.imode(),
                self.idtr(),
                self.isize(),
                self.admode(),
...
These bits are ignored when in dual-octal configuration (data on 8 bits and DMM├é┬á=├é┬á1) or 16├ó\u{80}\u{91}bit configuration (data exchanged over IO\\[15:0\\])."]
    ...
    impl defmt::Format for Cr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cr {{ en: {=bool:?}, abort: {=bool:?}, dmaen: {=bool:?}, tcen: {=bool:?}, dmm: {=bool:?}, fsel: {=bool:?}, fthres: {=u8:?}, teie: {=bool:?}, tcie: {=bool:?}, ftie: {=bool:?}, smie: {=bool:?}, toie: {=bool:?}, apms: {=bool:?}, pmm: {=bool:?}, fmode: {=u8:?}, msel: {=u8:?} }}",
                self.en(),
                self.abort(),
                self.dmaen(),
                self.tcen(),
...
for memories different from Micron. Others: Reserved."]
    ...
    impl defmt::Format for Dcr1 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Dcr1 {{ ckmode: {=bool:?}, frck: {=bool:?}, dlybyp: {=bool:?}, csht: {=u8:?}, devsize: {=u8:?}, mtyp: {=u8:?} }}",
                self.ckmode(),
                self.frck(),
                self.dlybyp(),
                self.csht(),
