...
pub mod regs {
    ...
    impl Cr {
        ...
        pub const fn cksrc(&self) -> super::vals::Cksrc {
            ...
            super::vals::Cksrc::from_bits(val as u8)
        }
        #[doc = "Clock source."]
        #[inline(always)]
    ...
    impl defmt::Format for Cr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Cr {{ ckpol: {:?}, depol: {:?}, rdypol: {:?}, edm: {:?}, enable: {=bool:?}, derdycfg: {:?}, cksrc: {:?}, dmaen: {=bool:?}, outen: {:?} }}" , self . ckpol () , self . depol () , self . rdypol () , self . edm () , self . enable () , self . derdycfg () , self . cksrc () , self . dmaen () , self . outen ())
        }
    }
    #[doc = "PSSI data register."]
    #[repr(transparent)]
...
pub mod vals {
    ...
    #[cfg_attr(feature = "defmt", derive(defmt::Format))]
    pub enum Cksrc {
        #[doc = "External clock (PSSI_PDCK in input)."]
        EXTERNAL_CLOCK = 0x0,
        #[doc = "Internal clock (PSSI_PDCK in output)."]
        INTERNAL_CLOCK = 0x01,
    }
    impl Cksrc {
        #[inline(always)]
        pub const fn from_bits(val: u8) -> Cksrc {
            unsafe { core::mem::transmute(val & 0x01) }
        }
        #[inline(always)]
        pub const fn to_bits(self) -> u8 {
    ...
    impl From<u8> for Cksrc {
        #[inline(always)]
        fn from(val: u8) -> Cksrc {
            Cksrc::from_bits(val)
        }
    }
    impl From<Cksrc> for u8 {
        #[inline(always)]
        fn from(val: Cksrc) -> u8 {
            Cksrc::to_bits(val)
        }
    }
    #[repr(u8)]
