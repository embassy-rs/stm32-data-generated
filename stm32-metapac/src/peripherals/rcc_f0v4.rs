...
from CFGR2 register. Refer to it for its meaning"]
    ...
    impl defmt::Format for Cir {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Cir {{ lsirdyf: {=bool:?}, lserdyf: {=bool:?}, hsirdyf: {=bool:?}, hserdyf: {=bool:?}, pllrdyf: {=bool:?}, hsi14rdyf: {=bool:?}, hsi48rdyf: {=bool:?}, cssf: {=bool:?}, lsirdyie: {=bool:?}, lserdyie: {=bool:?}, hsirdyie: {=bool:?}, hserdyie: {=bool:?}, pllrdyie: {=bool:?}, hsi14rdyie: {=bool:?}, hsi48rdyie: {=bool:?}, lsirdyc: {=bool:?}, lserdyc: {=bool:?}, hsirdyc: {=bool:?}, hserdyc: {=bool:?}, pllrdyc: {=bool:?}, hsi14rdyc: {=bool:?}, hsi48rdyc: {=bool:?}, cssc: {=bool:?} }}" , self . lsirdyf () , self . lserdyf () , self . hsirdyf () , self . hserdyf () , self . pllrdyf () , self . hsi14rdyf () , self . hsi48rdyf () , self . cssf () , self . lsirdyie () , self . lserdyie () , self . hsirdyie () , self . hserdyie () , self . pllrdyie () , self . hsi14rdyie () , self . hsi48rdyie () , self . lsirdyc () , self . lserdyc () , self . hsirdyc () , self . hserdyc () , self . pllrdyc () , self . hsi14rdyc () , self . hsi48rdyc () , self . cssc ())
        }
    }
    #[doc = "Clock control register"]
    #[repr(transparent)]
    ...
    impl defmt::Format for Cr2 {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Cr2 {{ hsi14on: {=bool:?}, hsi14rdy: {=bool:?}, hsi14dis: {=bool:?}, hsi14trim: {=u8:?}, hsi14cal: {=u8:?}, hsi48on: {=bool:?}, hsi48rdy: {=bool:?}, hsi48cal: {=u8:?} }}" , self . hsi14on () , self . hsi14rdy () , self . hsi14dis () , self . hsi14trim () , self . hsi14cal () , self . hsi48on () , self . hsi48rdy () , self . hsi48cal ())
        }
    }
    #[doc = "Control/status register (RCC_CSR)"]
    #[repr(transparent)]
...
pub mod vals {
    ...
    pub enum Sw {
        ...
        PLL1_P = 0x02,
        #[doc = "HSI48 used as system clock"]
        HSI48 = 0x03,
    }
    impl Sw {
        #[inline(always)]
