...
pub mod regs {
    ...
    impl Tafcr {
        ...
        #[doc = "Tamper 1 mapping"]
        #[inline(always)]
        pub const fn set_tamp1insel(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 16usize)) | (((val as u32) & 0x01) << 16usize);
        }
        ...
        #[doc = "Timestamp mapping"]
        #[inline(always)]
        pub const fn set_tsinsel(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 17usize)) | (((val as u32) & 0x01) << 17usize);
        }
    ...
    impl defmt::Format for Tafcr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Tafcr {{ tampe[0]: {=bool:?}, tampe[1]: {=bool:?}, tamptrg[0]: {:?}, tamptrg[1]: {:?}, tampie: {=bool:?}, tampts: {=bool:?}, tampfreq: {:?}, tampflt: {:?}, tampprch: {:?}, tamppudis: {:?}, tamp1insel: {=bool:?}, tsinsel: {=bool:?}, alarmouttype: {=bool:?} }}" , self . tampe (0usize) , self . tampe (1usize) , self . tamptrg (0usize) , self . tamptrg (1usize) , self . tampie () , self . tampts () , self . tampfreq () , self . tampflt () , self . tampprch () , self . tamppudis () , self . tamp1insel () , self . tsinsel () , self . alarmouttype ())
        }
    }
    #[doc = "Time register"]
    #[repr(transparent)]
