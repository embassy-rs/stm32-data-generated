...
pub mod regs {
    ...
    impl defmt::Format for Tafcr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Tafcr {{ tampe[0]: {=bool:?}, tampe[1]: {=bool:?}, tamptrg[0]: {:?}, tamptrg[1]: {:?}, tampie: {=bool:?}, tampts: {=bool:?}, tampfreq: {:?}, tampflt: {:?}, tampprch: {:?}, tamppudis: {:?}, tamp1insel: {=bool:?}, tsinsel: {=bool:?}, alarmouttype: {=bool:?} }}",
                self.tampe(0usize),
                self.tampe(1usize),
                self.tamptrg(0usize),
                self.tamptrg(1usize),
