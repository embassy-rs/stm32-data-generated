...
pub mod regs {
    ...
    impl defmt::Format for Icsr {
        fn format(&self, f: defmt::Formatter) {
            defmt :: write ! (f , "Icsr {{ wutwf: {=bool:?}, shpf: {=bool:?}, inits: {=bool:?}, rsf: {=bool:?}, initf: {=bool:?}, init: {=bool:?}, recalpf: {:?} }}" , self . wutwf () , self . shpf () , self . inits () , self . rsf () , self . initf () , self . init () , self . recalpf ())
        }
    }
    #[doc = "Masked interrupt status register"]
    #[repr(transparent)]
