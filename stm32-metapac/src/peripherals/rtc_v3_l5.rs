...
pub mod regs {
    ...
    impl defmt::Format for Misr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Misr {{ alrmf[0]: {:?}, alrmf[1]: {:?}, wutmf: {:?}, tsmf: {:?}, tsovmf: {:?}, itsmf: {:?} }}",
                self.alrmf(0usize),
                self.alrmf(1usize),
                self.wutmf(),
                self.tsmf(),
    ...
    impl defmt::Format for Scr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Scr {{ calrf[0]: {:?}, calrf[1]: {:?}, cwutf: {:?}, ctsf: {:?}, ctsovf: {:?}, citsf: {:?} }}",
                self.calrf(0usize),
                self.calrf(1usize),
                self.cwutf(),
                self.ctsf(),
    ...
    impl core::fmt::Debug for Smcr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Smcr")
                .field("alrdprot[0]", &self.alrdprot(0usize))
                .field("alrdprot[1]", &self.alrdprot(1usize))
    ...
    impl Smisr {
        ...
        #[doc = "Alarm x interrupt secure masked flag"]
        #[inline(always)]
        pub const fn set_alrmf(&mut self, n: usize, val: bool) {
            assert!(n < 2usize);
            let offs = 0usize + n * 1usize;
            self.0 = (self.0 & !(0x01 << offs)) | (((val as u32) & 0x01) << offs);
        }
    ...
    impl defmt::Format for Sr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Sr {{ alrf[0]: {:?}, alrf[1]: {:?}, wutf: {:?}, tsf: {:?}, tsovf: {:?}, itsf: {:?} }}",
                self.alrf(0usize),
                self.alrf(1usize),
                self.wutf(),
                self.tsf(),
