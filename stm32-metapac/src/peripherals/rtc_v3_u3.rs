...
pub mod regs {
    ...
    impl defmt::Format for Alrmr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Alrmr {{ su: {=u8:?}, st: {=u8:?}, msk1: {=bool:?}, mnu: {=u8:?}, mnt: {=u8:?}, msk2: {=bool:?}, hu: {=u8:?}, ht: {=u8:?}, pm: {:?}, msk3: {=bool:?}, du: {=u8:?}, dt: {=u8:?}, wdsel: {:?}, msk4: {=bool:?} }}",
                self.su(),
                self.st(),
                self.msk1(),
                self.mnu(),
    ...
    impl defmt::Format for Icsr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Icsr {{ wutwf: {=bool:?}, shpf: {=bool:?}, inits: {=bool:?}, rsf: {=bool:?}, initf: {=bool:?}, init: {=bool:?}, bin: {:?}, bcdu: {:?}, recalpf: {=bool:?} }}",
                self.wutwf(),
                self.shpf(),
                self.inits(),
                self.rsf(),
    ...
    impl defmt::Format for Misr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Misr {{ alrmf[0]: {=bool:?}, alrmf[1]: {=bool:?}, wutmf: {=bool:?}, tsmf: {=bool:?}, tsovmf: {=bool:?}, itsmf: {=bool:?}, ssrumf: {=bool:?} }}",
                self.alrmf(0usize),
                self.alrmf(1usize),
                self.wutmf(),
                self.tsmf(),
    ...
    impl defmt::Format for Sr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Sr {{ alrf[0]: {=bool:?}, alrf[1]: {=bool:?}, wutf: {=bool:?}, tsf: {=bool:?}, tsovf: {=bool:?}, itsf: {=bool:?}, ssruf: {=bool:?} }}",
                self.alrf(0usize),
                self.alrf(1usize),
                self.wutf(),
                self.tsf(),
    ...
    impl Tamptscr {
        ...
        #[doc = "Timestamp on external tamper TAMP1 event."]
        #[inline(always)]
        pub const fn set_tampts(&mut self, n: usize, val: bool) {
            assert!(n < 5usize);
            let offs = 0usize + n * 1usize;
            self.0 = (self.0 & !(0x01 << offs)) | (((val as u32) & 0x01) << offs);
        }
        ...
        #[doc = "Timestamp on internal tamper event."]
        #[inline(always)]
        pub const fn set_itampts(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 16usize)) | (((val as u32) & 0x01) << 16usize);
        }
    }
    ...
    impl defmt::Format for Tamptscr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Tamptscr {{ tampts[0]: {=bool:?}, tampts[1]: {=bool:?}, tampts[2]: {=bool:?}, tampts[3]: {=bool:?}, tampts[4]: {=bool:?}, itampts: {=bool:?} }}",
                self.tampts(0usize),
                self.tampts(1usize),
                self.tampts(2usize),
                self.tampts(3usize),
    ...
    impl Tsidr {
        ...
        #[doc = "Timestamp flag source identification."]
        #[inline(always)]
        pub const fn set_tsid(&mut self, val: u8) {
