...
bits must be configured before setting DMAEN to avoid a DMA request in receiver mode."]
    ...
    impl Cr2 {
        ...
        #[inline(always)]
        pub const fn fflush(&self) -> super::vals::Fflush {
            let val = (self.0 >> 3usize) & 0x01;
            super::vals::Fflush::from_bits(val as u8)
        }
        #[doc = "FIFO flush. This bit is set by software. It is always read as 0. This bit should be configured when the SAI is disabled."]
        #[inline(always)]
        pub fn set_fflush(&mut self, val: super::vals::Fflush) {
...
pub mod vals {
    ...
    impl Fflush {
        #[inline(always)]
        pub const fn from_bits(val: u8) -> Fflush {
            unsafe { core::mem::transmute(val & 0x01) }
        }
        #[inline(always)]
        pub const fn to_bits(self) -> u8 {
    ...
    impl From<u8> for Fflush {
        #[inline(always)]
        fn from(val: u8) -> Fflush {
            Fflush::from_bits(val)
        }
    }
    impl From<Fflush> for u8 {
        #[inline(always)]
        fn from(val: Fflush) -> u8 {
            Fflush::to_bits(val)
        }
    }
    #[repr(u8)]
