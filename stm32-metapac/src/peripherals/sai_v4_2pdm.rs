...
bits must be configured before setting DMAEN to avoid a DMA request in receiver mode."]
    ...
    impl defmt::Format for Cr1 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cr1 {{ mode: {:?}, prtcfg: {:?}, ds: {:?}, lsbfirst: {:?}, ckstr: {:?}, syncen: {:?}, mono: {:?}, outdriv: {:?}, saien: {=bool:?}, dmaen: {=bool:?}, nodiv: {=bool:?}, mckdiv: {:?}, osr: {:?}, mcken: {=bool:?} }}",
                self.mode(),
                self.prtcfg(),
                self.ds(),
                self.lsbfirst(),
...
bits and the audio block is operates as a receiver."]
    ...
    impl Pdmcr {
        ...
        #[doc = "Number of microphones"]
        #[inline(always)]
        pub const fn set_micnbr(&mut self, val: super::vals::Micnbr) {
